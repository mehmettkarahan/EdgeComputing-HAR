...
OUTPUT_DIR = "../results/mobilenet_har"
...
def build_model(model_name, num_classes):
    if model_name == "MobileNetV3Small":
        base_model = MobileNetV3Small(weights='imagenet', include_top=False, input_shape=IMAGE_SIZE + (3,))
        preprocess_func = mnv3_preprocess
    elif model_name == "MobileNetV3Large":
        base_model = MobileNetV3Large(weights='imagenet', include_top=False, input_shape=IMAGE_SIZE + (3,))
        preprocess_func = mnv3_preprocess
    elif model_name == "MobileNetV2":
        base_model = MobileNetV2(weights='imagenet', include_top=False, input_shape=IMAGE_SIZE + (3,))
        preprocess_func = mnv2_preprocess
    else:
...
# --- MAIN TRAINING LOOP ---
models_to_train = ["MobileNetV3Small", "MobileNetV2", "MobileNetV3Large"]
final_results = []
...
results_df.to_csv(os.path.join(OUTPUT_DIR, "mobilenets_results.csv"), index=False)
